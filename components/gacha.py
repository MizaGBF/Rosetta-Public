...
class Gacha():
    ...
    async def process(self, gtype : str, bid : Union[str, int], sub_id : int) -> tuple:
        try:
            # draw rate
            data = await self.bot.net.request("https://game.granbluefantasy.jp/gacha/provision_ratio/{}/{}/{}?PARAMS".format(gtype, bid, sub_id), account=self.bot.data.save['gbfcurrent'], expect_JSON=True)
    ...
    async def getScamRate(self, gtype : str, bid : Union[str, int]) -> dict:
        try:
            appear = (await self.bot.net.request("https://game.granbluefantasy.jp/gacha/provision_ratio/{}/{}/4?PARAMS".format(gtype, bid), account=self.bot.data.save['gbfcurrent'], expect_JSON=True))['appear']
            items = {}
            for e in appear:
    ...
    async def update(self) -> bool:
        ...
        try:
            c = self.bot.util.JST()
            #gacha page
            data = await self.bot.net.request("https://game.granbluefantasy.jp/gacha/list?PARAMS", account=self.bot.data.save['gbfcurrent'], expect_JSON=True)
