...
class BattleShipButton(disnake.ui.Button):
    ...
    def __init__(self : BattleShipButton, btype : int, target : str, row : int) -> None:
        super().__init__(
            style=(disnake.ButtonStyle.success if btype == 0 else disnake.ButtonStyle.danger),
            label=target,
            row=row
        )
        self.btype = btype
    ...
    async def callback(self : BattleShipButton, interaction: disnake.Interaction) -> None:
        # if state is valid (0 or 1) and the player turn corresponds to whoever triggered this callback
        if self.view.state >= 0 and self.view.players[self.view.state].id == interaction.user.id:
            ...
            if None not in self.view.input: # if both input slots are full
                # we "shoot" on the targeted cell and get the result
...
class BattleShip(BaseView):
    ...
    def __init__(
        ...
        # the player battleship grids (two of them, 5x5).
        # 0 = empty space, 1 = empty space + shot, 10 = ship, 11 = ship + shot
        self.grids : tuple[list[int], list[int]] = (
            [0 for i in range(20)] + [10 for i in range(5)],
            [0 for i in range(20)] + [10 for i in range(5)]
        )
        # randomize them
        ...
        for i in range(0, 2):
            self.embed.set_field_at(
                ...
                name=(
                    ...
                    if len(self.players[i].display_name) <= 10 else
                    self.players[i].display_name[:10] + '...'
                ),
                value=self.render(i)
            )
        # set message
    ...
    def shoot(self : BattleShip, value : str) -> int:
        ...
        match value[0]:
            ...
            case 'E': x = 4
        # check result
        # first check if it the cell has already been shot (i.e. equal to 1 or 11)
        if self.grids[opponent][x + y * 5] & 1 == 0:
