...
class Roles(commands.Cog):
    ...
    async def iam(
        ...
        role_name : str = commands.Param(description="The self-assignable role you want to get")
    ) -> None:
        ...
        if role_name.lower() not in roles: # check if the role exists in the self assignable list
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="Name `{}` not found in the self-assignable role list\nPlease check {}".format(
                        role_name,
                        self.bot.util.command2mention('role list')
                    ),
                    color=self.COLOR
                )
            )
                    ...
                    await inter.edit_original_message(
                        embed=self.bot.embed(
                            ...
                            description=(
                                ...
                                "Check if you have the role or contact a moderator."
                            ),
                            color=self.COLOR
                        )
                    )
    ...
    async def iamnot(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        role_name : str = commands.Param(description="The self-assignable role you want to remove")
    ) -> None:
    ...
    async def _list(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if len(roles) == 0:
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="No self-assignable roles available on this server",
                    color=self.COLOR
                )
            )
            return
    ...
    async def add(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        role_name : str = commands.Param(description="The self-assignable role you want to add to the list")
    ) -> None:
        ...
        if not self.bot.isMod(inter): # moderator check
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="Only moderators can make a role self-assignable",
                    color=self.COLOR
                )
            )
            return
        ...
        if role.name.lower() in self.bot.data.save['assignablerole'][gid]: # the role is already self assignable
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="Role `{}` is already a self-assignable role.\nDid you mean `/role remove` ?".format(
                        role_name
                    ),
                    color=self.COLOR
                )
    ...
    async def remove(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        role_name : str = commands.Param(description="The self-assignable role you want to remove from the list")
    ) -> None:
        ...
        if role.name.lower() not in self.bot.data.save['assignablerole'][gid]:
            # the role isn't in the self assignable list
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
