...
class GranblueFantasy(commands.Cog):
    ...
    """
    async def getGBFInfoTimers(self : GranblueFantasy, inter : disnake.ApplicationCommandInteraction, current_time : datetime) -> str:
    ...
    @gbf.sub_command()
    async def wiki(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, terms : str = commands.Param(description="Search expression")) -> None:
        """Search the GBF wiki"""
        ...
        if r is None or len(r[1]) == 0:
            await inter.edit_original_message(embed=self.bot.embed(title=title, description="**No results**\n[Click here to refine](https://gbf.wiki/index.php?title=Special:Search&search={})".format(quote(terms)), thumbnail="https://gbf.wiki/images/1/18/Vyrnball.png", color=self.COLOR))
    ...
    async def info(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        description : list[str] = ["{} Current Time is **{}**".format(self.bot.emote.get('clock'), self.bot.util.time(style=['d','t'])),
    ...
    async def maintenance(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        try:
            await inter.response.defer()
            description : str = (await self.bot.net.gbf_maintenance_status())[0] # simply retrieve maintenance string
            if len(description) > 0:
            ...
            match len(dates):
                ...
                case 2: # double timestamp event
                    ...
                    if current_time < start: # event hasn't started
                        ...
                        events[dates[0]].append("- **{}** ▫️ Ends in **{}** {}\n".format(event, self.bot.util.delta2str(end - current_time, 2), self.bot.util.time(end, style=['d'])))
                case _:
                    continue
        ...
        if len(msgs) == 0:
            ...
            if next is not None:
                ...
                if next[1].startswith("Update"):
                    ...
                    next_str = "event"
                msgs.append("{} Next {} approximately in **{}**\n".format(self.bot.emote.get('mark'), next_str, self.bot.util.delta2str(next[0] - current_time, 2)))
    ...
    async def gacha(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        try:
            ...
            if summary is None: raise Exception('No Gacha')
            await inter.edit_original_message(embed=self.bot.embed(author={'name':"Granblue Fantasy", 'icon_url':"https://prd-game-a-granbluefantasy.akamaized.net/assets_en/img/sp/touch_icon.png"}, description=summary[0], thumbnail=summary[1], color=self.COLOR))
        except Exception as e:
            if str(e) != 'No Gacha':
    ...
    async def setprofile(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, profile_id : int = commands.Param(description="A valid GBF Profile ID. Usurpation will result in ban.", ge=0, le=50000000)) -> None:
    ...
    async def processProfile(self : GranblueFantasy, pid : int|str, soup : BeautifulSoup) -> tuple[str, str, str]:
        ...
        # Trophy rarity
        rarity : str = "R"
        possible_headers : list[tuple[str, str]] = [("prt-title-bg-gld", "SSR"), ("prt-title-bg-slv", "SR"), ("prt-title-bg-nml", "R"), ("prt-title-bg-cpr", "R")]
        h : tuple[str, str]
        ...
        try:
            ...
            pass
        # Profile message
        comment : str = html.unescape(soup.find_all("div", class_="prt-other-comment")[0].string).replace('\t', '').replace('\n', '')
        if comment != "":
            if len(descs) > 0:
                descs.append(" ▫️ 💬 `")
            else:
        ...
        try:
            ...
            try:
                star.append(" **{}**".format(pushed.find("div", class_="prt-quality", recursive=True).get_text().strip())) # plus
            except:
                pass
            # add EMP level if it exists
            try:
                star.append(" ▫️ **{}** EMP".format(pushed.find("div", class_="prt-npc-rank", recursive=True).get_text().strip())) # emp
            except:
                pass
        ...
        # Support Summons
        try:
            script : BeautifulSoup = BeautifulSoup(soup.find("script", id="tpl-summon").get_text().replace(" <%=obj.summon_list.shift().viewClassName%>", ""), "html.parser")
            summon_lines : list[list[tuple[str, str]]] = [[] for i in range(7*2)] # 2 lines for each element + misc. Misc is located last. Also, we support up to 4 summons per element.
            i : int = 0
            j : int
            x : int
        ...
        try:
            thumbnail = soup.find_all("img", class_="img-pc")[0]['src'].replace("/po/", "/talk/").replace("/img_low/", "/img/")
        except:
            pass
    ...
    @profile.sub_command()
    async def see(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, target : str = commands.Param(description="Either a valid GBF ID, discord ID or mention", default="")) -> None:
    ...
    @_utility.sub_command()
    async def xp(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, start_level : int = commands.Param(description="Starting Point of the calcul", ge=1, le=149, default=1), end_level : int = commands.Param(description="Final Point of the calcul", ge=1, le=150, default=1)) -> None:
    ...
    @_utility.sub_command()
    async def kirinanima(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, talisman : int = commands.Param(description="Talisman count", ge=0, le=100000, default=0), ream : int = commands.Param(description="Ream count", ge=0, le=100000, default=0), silver_anima : int = commands.Param(description="Silver Anima count", ge=0, le=100000, default=0), omega_anima : int = commands.Param(description="Omega Anima count", ge=0, le=100000, default=0)) -> None:
    ...
    async def koregra(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        await inter.response.defer()
        c : datetime = self.bot.util.JST()
        m : datetime = c.replace(day=1, hour=12, minute=0, second=0, microsecond=0) # rough estimate of when it's released
    ...
    @check.sub_command()
    async def granblues(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction, episode : int = commands.Param(description="A Grand Blues! episode number", default=1, ge=1, le=99999)) -> None:
    ...
    async def crystal(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        try:
            ...
            if c > end or self.bot.data.save['extra'].get('campaign/dividecrystal', {}).get('wave', 9999) > maxwave:
                ...
                try:
                    # access endpoint
                    data : str = unquote((await self.bot.net.requestGBF("campaign/dividecrystal/content/index", expect_JSON=True))['data'])
                ...
                if crystal <= 0:
                    ...
                    msgs : list[str] = ["{:} **{:,}** crystals remaining (Average **{:}** players/day, at {:,} crystals average).\n".format(self.bot.emote.get('crystal'), crystal, self.bot.util.valToStr(players), avg_completion_crystal)]
                    msgs.append("{} Event is ending in **{}**.\n".format(self.bot.emote.get('clock'), self.bot.util.delta2str(end - c, 2)))
                    elapsed : timedelta = c - start
                    duration : timedelta = end - start
                    progresses : list[float] = [100 * (consumed / available_crystal), 100 * (elapsed.days * 86400 + elapsed.seconds) / (duration.days * 86400 + duration.seconds)]
                    msgs.append("Progress ▫️ **{:.2f}%** {:} ▫️ **{:.2f}%** {:} ▫️ ".format(progresses[0], self.bot.emote.get('crystal'), progresses[1], self.bot.emote.get('clock')))
                    if progresses[1] > progresses[0]:
                        ...
                        msgs.append("Estimating between **{:,}** and **{:,}** bonus crystals/player at the end.".format(int(leftover / eligible), int(leftover / 550000)))
                        if footer != "":
                            footer += " - "
                        footer += "Assuming ~{} eligible players.".format(self.bot.util.valToStr(eligible))
                    else:
                        msgs.append("⚠️\n")
                        t : timedelta = timedelta(seconds = (duration.days * 86400 + duration.seconds) * (100 - (progresses[1] * 100 / progresses[0])) / 100)
                        msgs.append("Crystals will run out in **{}** at current pace.".format(self.bot.util.delta2str(end - t - c, 2)))
                    msg = "".join(msgs)
        except Exception as e:
        ...
        try:
            ...
            if c > end:
                ...
                for k, v in data["total"].items():
                    if v >= data['goal']:
                        msgs.append("{:} ▫️ **{:,}** {:}".format(self.bot.emote.get(elems.get(k, k)), v, self.bot.emote.get('crown')))
                    else:
                        msgs.append("{:} ▫️ **{:,}** ({:.2f}%)".format(self.bot.emote.get(elems.get(k, k)), v, 100*v/data['goal']))
                msgs.append("{} Event is ending in **{}**.".format(self.bot.emote.get('clock'), self.bot.util.delta2str(end - c, 2)))
                msg = '\n'.join(msgs)
        except Exception as e:
            msg = "An error occured, try again later."
