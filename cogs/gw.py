...
class GuildWar(commands.Cog):
        ...
        if crew['private']:
            ...
            if gwstate:
                ...
                if gwid and len(players) - unranked > 0:
                    ...
                    if median > 0:
                        description += " | Med. **{}**".format(self.bot.util.valToStr(median, 2))
                    if unranked > 0:
    ...
    async def _players(self, gbfgdata : dict) -> tuple:
        try:
            ...
            for k in dancho_ranking: # build dancho list (for emote)
                dancho_list.append(k[3])
            await asyncio.sleep(0)
