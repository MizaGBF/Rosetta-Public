...
class Sparking(commands.Cog):
    ...
    """
    def _estimate(self : Sparking, r : float, timestamp : datetime|None) -> tuple[datetime, datetime, tuple[int, int], datetime]:
    ...
    async def nickname(self : commands.SubCommand, inter : disnake.ApplicationCommandInteraction) -> None:
        ...
        if inter.guild is None:
            ...
            m : disnake.Member|None = await inter.guild.get_or_fetch_member(inter.author.id) # fetch the author member object in the guild
    ...
    async def _ranking(self : Sparking, inter : disnake.ApplicationCommandInteraction, guild : disnake.Guild) -> tuple[str|None, int|None]:
        ...
        i : int = 0
        emotes : dict[int, str|disnake.Emoji] = {0:self.bot.emote.get('SSR'), 1:self.bot.emote.get('SR'), 2:self.bot.emote.get('R')} # emotes used for the top 3
        msgs : list[str] = []
    ...
    @commands.max_concurrency(4, commands.BucketType.default)
    async def seespark(self : commands.user_command, inter : disnake.UserCommandInteraction, member: disnake.Member) -> None:
