...
class YouCrew(commands.Cog):
    """Owner only."""
    guild_ids : list[int]
    if CREW_SERVER_ID is None:
        guild_ids = [] # CREW_SERVER_ID is defined in cogs/init
    else:
    ...
    """
    async def checkGWBuff(self : YouCrew) -> None:
        # retrieve cog
        gwcog : commands.Cog = self.bot.get_cog('GuildWar')
        if gwcog is None:
            # add warning in log, just in case
            self.bot.logger.push(
                "[TASK] 'you:buff' Task Cancelled, the 'GuildWar' Cog is missing.",
        ...
        try:
            ...
            channel : disnake.Channel|None = self.bot.get_channel(
                self.bot.data.config['ids'].get('you_announcement', 0)
            )
            if channel is None:
            ...
            buff_role : tuple[int, int] = (
                [guild.get_role(self.bot.data.config['ids'].get('you_atkace', 0)), 'atkace'],
                [guild.get_role(self.bot.data.config['ids'].get('you_deface', 0)), 'deface']
            )
            ...
            while self.bot.data.save['gw']['state'] and (len(self.bot.data.save['gw']['buffs']) > 0 or len(msgs) != 0):
                ...
                current_time : datetime = self.bot.util.JST() + timedelta(seconds=32)
                if (len(self.bot.data.save['gw']['buffs']) > 0
                        and current_time >= self.bot.data.save['gw']['buffs'][0][0]):
                    msgs = []
                    ...
                    if (current_time - self.bot.data.save['gw']['buffs'][0][0]) < timedelta(seconds=200):
                        ...
                        # add Link
                        msgs.append(
                            "\nhttps://game.granbluefantasy.jp/#event/teamraid{}/guild_ability".format(
                    ...
                    if len(self.bot.data.save['gw']['buffs']) > 0: # if a buff is remaining, sleep until its time
                        d : timedelta = self.bot.data.save['gw']['buffs'][0][0] - current_time
                        if d.seconds > 1:
        ...
        await self.bot.send(
            ...
            embed=self.bot.embed(
                title="User task ended",
                description="you:buff",
    ...
    """
    def setBuffTask(self : YouCrew, state : bool) -> None:
    ...
    """
    def getNextBuff(self : YouCrew, inter : disnake.GuildCommandInteraction) -> str:
        # check if gw is on going and the server is the right one
        if (self.bot.data.save['gw']['state'] is True
                and inter.guild.id == self.bot.data.config['ids'].get('you_server', 0)):
            current_time : datetime = self.bot.util.JST() # get current time
            ...
            for b in self.bot.data.save['gw']['buffs']: # iterate until closest buff to current time
                if not b[3] and current_time < b[0]:
                    # build message
                    msgs : list[str] = [
                        "{} Next buffs in **{}** (".format(
                            self.bot.emote.get('question'),
                            self.bot.util.delta2str(b[0] - current_time, 2)
                        )
                    ]
                    if b[1]:
                        msgs.append(
                            "Attack {}, Defense {}".format(
                                self.bot.emote.get('atkace'),
                                self.bot.emote.get('deface')
                            )
                        )
                        if b[2]:
    ...
    """
    async def searchScoreForTracker(
        self : YouCrew,
        day : int,
        crews : list[int|str]
    ) -> list[tuple[str, int, float]]:
        infos : list[tuple[str, int, float]] = []
        conn : sqlite3.Connection = sqlite3.connect('temp.sql') # open temp.sql
        ...
        for sid in crews: # retrieve both crews data
            ...
            # name, score of the day, top speed
            infos.append([data[0][2], data[0][d[day]] - data[0][d[day] - 1], data[0][8]])
            await asyncio.sleep(0)
        c.close()
        conn.close()
    ...
    Returns
    ----------
    bytes: image binary d ata, None if error
    """
    async def drawChart(self : YouCrew, plot : list) -> bytes|None:
        if len(plot) == 0:
            return None # no plot data
        i : int
        # make a white RGB image, 800x600px
        img : Image = Image.new("RGB", (800, 600), (255, 255, 255))
        d : ImageDraw.Draw = ImageDraw.Draw(img)
        # load our font for texts
        font : ImageFont = ImageFont.truetype("assets/font.ttf", 14)
        # y grid lines
        for i in range(0, 4):
        ...
        deltax : int = (deltax.seconds + deltax.days * 86400)
        if deltax <= 0:
            return None
        tvar = minx
        for i in range(0, 11):
        ...
        # scores curves
        lines : tuple[list[tuple[int|float, int|float]], list[tuple[int|float, int|float]]] = ([], [])
        x : int|float
        y : int|float
        for p in plot:
            x = p[0] - minx
    ...
    """
    async def updateTracker(self : YouCrew, t : datetime, day : int) -> None:
        you_id : str|None = self.bot.ranking.gbfgcrews.get('you', None) # our id
        if you_id is None:
            return
        # check tracker state
        if self.bot.data.save['matchtracker'] is None:
        ...
        if self.bot.data.save['matchtracker']['day'] != day: # new day, reset data
            ...
            self.bot.data.pending = True
        # Retrieve scores
        infos : list[tuple[str, int, float]] = await self.searchScoreForTracker(
            day,
            [you_id, self.bot.data.save['matchtracker']['id']]
        )
        # Make tracker copy, to be safe
        newtracker : JSON = self.bot.data.save['matchtracker'].copy()
        if newtracker['init']: # it's initialized
            d : datetime = t - newtracker['last'] # time delta
            minute : int = d.seconds // 60 # elapsed minutes in that delta
            ...
            if minute != 0:
                speed : list[float] = [
                    (infos[0][1] - newtracker['scores'][0]) / minute,
                    (infos[1][1] - newtracker['scores'][1]) / minute
                ]
                if speed[0] > newtracker['top_speed'][0]:
                    newtracker['top_speed'][0] = speed[0]
                if speed[1] > newtracker['top_speed'][1]:
                    newtracker['top_speed'][1] = speed[1]
        ...
        if len(newtracker['plot']) > 1: # generate a chart
            try:
                imgdata : bytes|None = await self.drawChart(newtracker['plot'])
    ...
    async def you(self : commands.slash_command, inter : disnake.GuildCommandInteraction) -> None:
        """Command Group"""
        pass

    @you.sub_command()
    async def buff(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        """Check when is the next GW buff ((You) Server Only)"""
        try:
            await inter.response.defer()
            d : str = self.getNextBuff(inter) # retrieve next buff string
            if d != "":
    ...
    async def lead(
        ...
        opponent : str = commands.Param(description="Opponent ID to set it (Mod Only)", default="")
    ) -> None:
        ...
        if opponent != "" and self.bot.isMod(inter): # opponent set mode (MUST BE A MODERATOR)
            ...
            if self.bot.data.save['matchtracker'] is None or not self.bot.data.save['matchtracker']['init']:
                ...
                if self.bot.data.save['matchtracker']['last'].hour < 7: # if match is on going
                    end_time = self.bot.data.save['matchtracker']['last'].replace(
                        hour=0,
                        minute=0,
                        second=0,
                        microsecond=0
                    ...
                    end_time = self.bot.data.save['matchtracker']['last'].replace(
                        day=self.bot.data.save['matchtracker']['last'].day + 1,
                        hour=0,
                        minute=0,
                        second=0,
                        microsecond=0
                ...
                remaining : timedelta = end_time - self.bot.data.save['matchtracker']['last']
                ...
                for i in range(2): # our crew, opponent crew
                    ...
                    msgs.append("[{:}](https://game.granbluefantasy.jp/#guild/detail/{:}) ▫️ **{:,}**".format(
                        self.bot.data.save['matchtracker']['names'][i],
                        (you_id if i == 0 else self.bot.data.save['matchtracker']['id']),
                        self.bot.data.save['matchtracker']['scores'][i]
                    ))
                    ...
                    msgs.append("\n**Speed** ▫️ Now {}/m".format(
                        self.bot.util.valToStr(
                            self.bot.data.save['matchtracker']['speed'][i],
                            2
                        )
                    ))
                    if (self.bot.data.save['matchtracker']['speed'][i]
                            >= self.bot.data.save['matchtracker']['top_speed'][i]):
                        msgs.append(" ▫️ **Top {}/m** {}".format(
                            self.bot.util.valToStr(
                                self.bot.data.save['matchtracker']['top_speed'][i],
                                2
                            ),
                    ...
                    max_speed : float = max(
                        self.bot.data.save['matchtracker']['max_speed'][i],
                        self.bot.data.save['matchtracker']['top_speed'][i]
                    )
                    if self.bot.data.save['matchtracker']['speed'][i] >= max_speed:
                        msgs.append(" ▫️ **Max {}/m** {}".format(
                            self.bot.util.valToStr(max_speed, 2),
                            ":white_check_mark:" if i == 0 else ":warning:"
                        ))
                    else:
                    ...
                    if end_time > self.bot.data.save['matchtracker']['last']:
                        ...
                        current_estimation : int|float = (
                            self.bot.data.save['matchtracker']['scores'][i]
                            + self.bot.data.save['matchtracker']['speed'][i]
                            * remaining.seconds // 60
                        )
                        max_estimation : int|float = (
                            self.bot.data.save['matchtracker']['scores'][i]
                            + max_speed
                            * remaining.seconds // 60
                        )
                        top_estimation : int|float = (
                            self.bot.data.save['matchtracker']['scores'][i]
                            + self.bot.data.save['matchtracker']['top_speed'][i]
                            * remaining.seconds // 60
                        )
                        msgs.append("\n**Estimation** ▫ Now {} ▫️ Top {} ▫️ Max {}".format(
                            self.bot.util.valToStr(current_estimation, 3),
                            self.bot.util.valToStr(top_estimation, 3),
                            self.bot.util.valToStr(max_estimation, 3)
                        ))
                    else:
                ...
                # calculate lead
                lead : int = (
                    self.bot.data.save['matchtracker']['scores'][0]
                    ...
                    if lead_speed is not None and lead_speed != 0:
                        try:
                            ...
                            if lead_speed < 0: # negative lead speed
                                # calc remaining minutes to exshaut current lead
                                minute : int = abs(lead) / abs(lead_speed)
                                d : datetime = (
                                    self.bot.data.save['matchtracker']['last']
                                    + timedelta(seconds=minute * 60) # switch time
                                )
                                e : datetime = ct.replace(
                                    hour=0,
                                    minute=0,
                                    second=0,
                                    microsecond=0
                                ) + timedelta(days=1) # day end time
                                if e > d: # the lead WILL switch before the end
                                    if lead > 0:
                                        ...
                                        msgs.append("\n:white_check_mark: ")
                                    if d >= ct:
                                        msgs.append("The Lead switches in **{}** at current speeds".format(
                                            self.bot.util.delta2str(d - ct)
                                        ))
                                    else:
                            ...
                            if not lead_will_switch and lead > 0:
                                if self.bot.data.save['matchtracker']['scores'][0] > top_estimation: # winning case
                                    msgs.append("\n:confetti_ball: Opponent can't catch up ")
                                    if (self.bot.data.save['matchtracker']['max_speed'][1]
                                            > self.bot.data.save['matchtracker']['top_speed'][1]):
                                        msgs.append("but **can still go faster**, be careful")
                                    else:
                                        msgs.append("without surpassing their **max speed**")
                                elif self.bot.data.save['matchtracker']['scores'][0] > current_estimation: # ahead case
                                    msgs.append("\n:white_check_mark: Opponent can't catch up")
                                    msgs.append("without increasing their **current speed**")
                ...
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="{} **Guild War {} ▫️ Day {}**".format(
                            ...
                            self.bot.data.save['matchtracker']['gwid'],
                            self.bot.data.save['matchtracker']['day']
                        ),
                        description="".join(msgs),
                        timestamp=self.bot.util.UTC(),
                        thumbnail=self.bot.data.save['matchtracker'].get('chart', None),
                        color=self.COLOR
                    )
                )
    ...
    @you.sub_command()
    async def honor(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        """Retrieve (You) members's honor ((You) Server Only)"""
        await inter.response.defer(ephemeral=True)
        cid : str|None = self.bot.ranking.gbfgcrews.get("(you)", None) # retrieve (You) id
        if cid is None: # id not found
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="(You) Honor List",
                    description="Crew not found",
                    color=self.COLOR
                )
            )
        else:
            data : GBFGData|None = await self.bot.get_cog('GuildWar').updateGBFGData() # get gbfg data
            if data is None or cid not in data or len(data[cid][-1]) == 0:
                ...
                players : list[str|int|list[str|int]] = data[cid][-1]
                await asyncio.sleep(0)
                # query db
                data : GWDBSearchResult = await self.bot.ranking.searchGWDB("(" + ",".join(players) + ")", 4)
                # send result
                if data[1] is None:
                    await inter.edit_original_message(
                        embed=self.bot.embed(
                            title="(You) Honor List",
                            description="No GW data found",
                            color=self.COLOR
                        )
                    )
                else:
                    players : dict[int, str] = {int(p) : "n/a" for p in players}
                    p : Score
                    for p in data[1]:
                        if p.id in players:
