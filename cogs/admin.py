...
class Admin(commands.Cog):
    ...
    """
    async def status(self : Admin) -> None:
        await self.bot.change_presence(
            status=disnake.Status.online,
            activity=disnake.activity.Game(name="I rebooted in the last 30 minutes")
        )
        num : int = 0
        while True:
            try:
                ...
                if num == 0: # status change every two hours and refresh GBF account at the same time
                    await self.bot.change_presence(
                        status=disnake.Status.online,
                        activity=disnake.activity.Game(name=random.choice(self.bot.data.config['games']))
                    )
            ...
            if inter.bot.isOwner(inter):
                ...
                await inter.response.send_message(
                    embed=inter.bot.embed(
                        title="Error",
                        description="You lack the permission to use this command"
                    ),
                    ephemeral=True
                )
                return False
    ...
    """
    async def guildList(self : Admin) -> None:
        msgs : list[str] = []
        length : int = 0
        # iterate over guilds
        s : disnake.Guild
    ...
    async def answer_callback(self, modal : disnake.ui.Modal, inter : disnake.ModalInteraction) -> None:
        try:
            ...
            await target.send(
                embed=self.bot.embed(
                    title="Answer to your Bug Report ▫️ " + inter.text_values['title'],
                    description=inter.text_values['description'],
                    image=inter.text_values['image'],
                    color=self.COLOR
                )
            )
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Information",
                    description='Answer sent with success',
                    color=self.COLOR
                )
            )
        except Exception as e:
    ...
    async def spark(
        ...
        await inter.edit_original_message(
            embed=self.bot.embed(
                title="Unbanned user for roll ranking",
                color=self.COLOR
            )
        )
    ...
    async def reboot(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        self._shutdown()
        # Note: Assume a Watchtower or other similar system is in place.
        # Else, the bot will just shutdown, not reboot

    @_bot.sub_command()
    async def avatar(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        filename : str = commands.Param(description="Filename of the asset to use")
    ) -> None:
    ...
    async def load(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        drive : int = commands.Param(description="Add 1 to use the file from the drive", default=0)
    ) -> None:
        ...
        if drive != 0: # download the save file from the drive.
            if self.bot.drive.load() is False:
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="Failed to retrieve save.json on the Google Drive",
                        color=self.COLOR
                    )
                )
                return
        ...
        if self.bot.data.loadData():
            try:
                self.bot.get_cog('YouCrew').setBuffTask(True) # reload gw buff task
            except:
                pass
            ...
            await self.bot.send(
                ...
                embed=self.bot.embed(
                    title=inter.me.name,
                    description="save.json reloaded",
                    color=self.COLOR
                )
            )
        else:
            await self.bot.send(
                ...
                embed=self.bot.embed(
                    title=inter.me.name,
                    description="save.json loading failed",
                    color=self.COLOR
                )
            )
        await inter.edit_original_message(
            embed=self.bot.embed(
                title="The command finished running",
                color=self.COLOR
            )
        )
    ...
    async def get(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        filename: str = commands.Param(description="Path to a local file")
    ) -> None:
        ...
        try:
            ...
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="File sent to the debug channel",
                    color=self.COLOR
                )
            )
        except Exception as e:
    ...
    @data.sub_command()
    async def clearprofile(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        gbf_id : int = commands.Param(description="A valid GBF Profile ID", ge=0)
    ) -> None:
    ...
    async def maintset(
        ...
        hour : int = commands.Param(ge=0, le=23),
        duration : int = commands.Param(ge=0)
    ) -> None:
    ...
    async def streamset_callback(self : Admin, modal : disnake.ui.Modal, inter : disnake.ModalInteraction) -> None:
        ...
        await inter.edit_original_message(
            embed=self.bot.embed(
                ...
                description="Stream command sets to\n`{}`\n`{}`".format(
                    self.bot.data.save['stream']['title'],
    ...
    async def streamset(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if self.bot.data.save['stream'] is None:
            ...
            data = {
                ...
                'content':self.bot.data.save['stream']['content'],
                'time':self.bot.data.save['stream']['time']
            }
        await self.bot.singleton.make_and_send_modal(
            inter,
    ...
    async def time(
        ...
        day : int = commands.Param(ge=1, le=31),
        month : int = commands.Param(ge=1, le=12),
        year : int = commands.Param(ge=2021),
        hour : int = commands.Param(ge=0, le=23)
    ) -> None:
        ...
        try:
            ...
            if self.bot.data.save['stream'] is None: # the stream MUST be set beforehand
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="Stream Error",
                        description="No Stream set.\nSet one first with {}.".format(
                            self.bot.util.command2mention("owner stream set")
                        ),
                        color=self.COLOR
                    )
                )
            else: # set the time value
                self.bot.data.save['stream']['time'] = datetime.now().replace(
                    year=year,
                    month=month,
                    day=day,
                    hour=hour,
    ...
    async def scheduleadd(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        name : str = commands.Param(description="Entry name"),
        start : str = commands.Param(description="Start date (DD/MM/YY format)"),
        start_hour : int = commands.Param(description="Start Hour UTC (Default is 8)", default=8, ge=0, le=23),
        end : str = commands.Param(description="End date (DD/MM/YY format)", default=""),
        end_hour : int = commands.Param(description="Ending Hour UTC (Default is 8)", default=8, ge=0, le=23)
    ) -> None:
        ...
        try:
            ...
            timestamps : list[int] = [int(
                datetime.strptime(start, '%d/%m/%y').replace(
                    hour=start_hour,
                    minute=0,
                    second=0
                ).timestamp()
            ...
            if end != "":
                timestamps.append(int(
                    datetime.strptime(end, '%d/%m/%y').replace(
                        hour=end_hour,
                        minute=0,
                        second=0
                    ).timestamp()
            ...
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Schedule Update",
                    description=(
                        "Error, did you set the date in the proper format"
                        " (DD/MM/YY)?\nException:\n`{}`"
                    ).format(
                        self.bot.pexc(e)
                    ),
    ...
    async def see(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if not self.bot.net.has_account():
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="GBF Account status",
                    description="No accounts set",
                    color=self.COLOR
                )
            )
        else:
            ...
            if r is None or not self.bot.net.is_account_valid():
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="GBF Account status",
                        description=(
                            "Account is down or GBF is unavailable\n"
                            "User ID: `{}`\n"
                            "Cookie: `{}`\n"
                            "User-Agent: `{}`"
                        ).format(
    ...
    async def set(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        uid : int = commands.Param(default=0),
        ck : str = commands.Param(default=""),
        ua : str = commands.Param(default="")
    ) -> None:
    ...
    async def edit(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        uid : int = commands.Param(description="User ID", ge=-1, default=-1),
        ck : str = commands.Param(description="Cookie", default=""),
        ua : str = commands.Param(description="User-Agent", default="")
    ) -> None:
        ...
        if not self.bot.net.has_account():
            await inter.edit_original_message(
                embed=self.bot.embed(
                    ...
                    description="Use {} instead.".format(
                        self.bot.util.command2mention('owner account set')
                    ),
                    color=self.COLOR
                )
            )
            ...
            if uid is None and ck is None and ua is None:
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="Error",
                        description="You didn't set any valid value.\nNothing has been modified.",
                        color=self.COLOR
                    )
                )
    ...
    async def dbset(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        db_id : int = commands.Param(description="Dread Barrage ID", ge=0, le=999),
        element : str = commands.Param(
            description="Dread Barrage Element Advantage",
            autocomplete=[
                'fire', 'water', 'earth', 'wind', 'light', 'dark'
            ]
        ),
        day : int = commands.Param(description="Dread Barrage Start Day", ge=1, le=31),
        month : int = commands.Param(description="Dread Barrage Start Month", ge=1, le=12),
        year : int = commands.Param(description="Dread Barrage Start Year", ge=2021),
        extra_days : int = commands.Param(
            description="Number of days to increase the event duration (0 - 2)",
            ge=0, le=2, default=0
        )
    ) -> None:
        ...
        try:
            ...
            self.bot.data.save['dread']['dates']["Day 1"] = self.bot.util.UTC().replace(
                year=year,
                month=month,
                day=day,
                hour=19,
            ...
            self.bot.data.save['dread']['dates']["Day 2"] = self.bot.data.save['dread']['dates']["Day 1"] + ONE_DAY
    ...
    async def enable(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if self.bot.data.save['dread']['state'] is True:
            ...
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="No Dread Barrage available in my memory",
                    color=self.COLOR
                )
            )
    ...
    async def enable__(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if self.bot.data.save['gw']['state'] is True:
            ...
            self.bot.data.pending = True
            try:
                self.bot.get_cog('YouCrew').setBuffTask(True)
            except:
                pass
            ...
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="No Guild War available in my memory",
                    color=self.COLOR
                )
            )
    ...
    async def forceupdateranking(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if self.bot.data.save['gw']['state']:
            ...
            await self.bot.ranking.retrieve_ranking(
    ...
    async def gwset(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        gw_id : int = commands.Param(description="Guild War ID", ge=0, le=999),
        element : str = commands.Param(
            description="Guild War Element Advantage",
            autocomplete=[
                'fire', 'water', 'earth', 'wind', 'light', 'dark'
            ]
        ),
        day : int = commands.Param(description="Guild War Start Day", ge=1, le=31),
        month : int = commands.Param(description="Guild War Start Month", ge=1, le=12),
        year : int = commands.Param(description="Guild War Start Year", ge=2021)
    ) -> None:
        ...
        try:
            ...
            # const
            ONE_DAY : timedelta = timedelta(days=1)
            HOUR_17 : timedelta = timedelta(seconds=61200)
            HOUR_36 : timedelta = timedelta(days=1, seconds=43200)
            # build the calendar
            self.bot.data.save['gw']['dates'] = {}
            self.bot.data.save['gw']['dates']["Preliminaries"] = self.bot.util.UTC().replace(
                year=year,
                month=month,
                day=day,
                hour=19,
            ...
            self.bot.data.save['gw']['dates']["Interlude"] = (
                self.bot.data.save['gw']['dates']["Preliminaries"] + HOUR_36
            )
            self.bot.data.save['gw']['dates']["Day 1"] = self.bot.data.save['gw']['dates']["Interlude"] + ONE_DAY
            self.bot.data.save['gw']['dates']["Day 2"] = self.bot.data.save['gw']['dates']["Day 1"] + ONE_DAY
            self.bot.data.save['gw']['dates']["Day 3"] = self.bot.data.save['gw']['dates']["Day 2"] + ONE_DAY
            self.bot.data.save['gw']['dates']["Day 4"] = self.bot.data.save['gw']['dates']["Day 3"] + ONE_DAY
            self.bot.data.save['gw']['dates']["Day 5"] = self.bot.data.save['gw']['dates']["Day 4"] + ONE_DAY
            self.bot.data.save['gw']['dates']["End"] = (
                self.bot.data.save['gw']['dates']["Day 5"] + HOUR_17
            )
            ...
            # Prelims, double buff
            self.bot.data.save['gw']['buffs'].append(
                [
                    self.bot.data.save['gw']['dates']["Preliminaries"]
                    + timedelta(seconds=7200 - 300),
                    True, True, True, True # warning, double
                ]
            )
            self.bot.data.save['gw']['buffs'].append(
                [
            ...
            # Usual days
            time_lists : list[datetime] = [
                self.bot.data.save['gw']['dates']["Preliminaries"] + timedelta(seconds=43200),
                self.bot.data.save['gw']['dates']["Interlude"],
                self.bot.data.save['gw']['dates']["Day 1"],
                self.bot.data.save['gw']['dates']["Day 2"],
                self.bot.data.save['gw']['dates']["Day 3"],
                self.bot.data.save['gw']['dates']["Day 4"]
            ]
            date : datetime
            for date in time_lists:
                ...
                self.bot.data.save['gw']['buffs'].append(
                    [
                        date + timedelta(seconds=3600 - 300),
                        False, True, True, False
                    ]
                )
                self.bot.data.save['gw']['buffs'].append([date + timedelta(seconds=3600), False, True, False, False])
                self.bot.data.save['gw']['buffs'].append(
                    [
                        date + timedelta(seconds=54000 - 300),
                        True, True, True, False
                    ]
                )
                self.bot.data.save['gw']['buffs'].append([date + timedelta(seconds=54000), True, True, False, False])
    ...
    @gw.sub_command()
    async def movepreliminaries(
        self : commands.SubCommand,
        inter : disnake.GuildCommandInteraction,
        delta_prelim_hour : int = commands.Param(description="Number of hours to add to prelims", ge=1, le=999),
        delta_interlude_hour : int = commands.Param(
            description="Number of hours to add to interlude",
            ge=0, le=999, default=0
        )
    ) -> None:
        ...
        if self.bot.data.save['gw']['state']:
            if self.bot.data.save['gw']['dates']["Day 1"] == self.bot.data.save['gw']['dates']["Interlude"]:
                await inter.edit_original_message(
                    embed=self.bot.embed(
                        title="The move is already affective",
                        color=self.COLOR
                    )
                )
            else:
    ...
    async def check(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        try:
            ...
            for b in self.bot.data.save['gw']['buffs']:
                ...
                if b[2]:
                    msgs.append('[FO Buffs] ')
                if b[3]:
                    msgs.append('[Warning] ')
                if b[4]:
                    msgs.append('[Double duration] ')
                msgs.append('\n')
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="{} Guild War (You) Buff debug check".format(
                        self.bot.emote.get('gw')
                    ),
                    description=''.join(msgs),
                    color=self.COLOR
    ...
    async def skip(self : commands.SubCommand, inter : disnake.GuildCommandInteraction) -> None:
        ...
        if not self.bot.data.save['gw']['skip']:
            ...
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Next buffs will be skipped",
                    color=self.COLOR
                )
            )
        else:
            await inter.edit_original_message(
                embed=self.bot.embed(
                    title="Error",
                    description="The next set of buffs is already beind skipped",
                    color=self.COLOR
                )
            )
    ...
    async def roulette(
        ...
        await inter.edit_original_message(
            embed=self.bot.embed(
                title="Roulette Simulator settings",
                description="".join(msgs),
                color=self.COLOR
            )
        )
