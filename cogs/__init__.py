...
def loadCogFile(
    ...
    try:
        ...
        with open(path_filename, mode='r', encoding='utf-8') as py:
            ...
            for node in classes:
                try:
                    ...
                    _class : commands.cog.CogMeta = getattr(
                        import_module(relative + module_name, package=package),
                        node.name
                    )
        ...
        if 'No such file or directory:' in str(e):
            bot.logger.pushError(
                (
                    "[COG] {} is missing and will be ignored.\n"
                    "Ignore this message if it's intended."
                ).format(path_filename),
                send_to_discord=not silent and filename != "test.py",
                level=bot.logger.WARNING
            )
        else:
            bot.logger.pushError(
...
def load(bot : DiscordBot) -> tuple[int, int]:
    # Silent flag
    ...
    try: # try to set debug server id for modules needing it
        ...
        bot.logger.push(
            "[WARNING] 'debug_server' ID not set in 'config.json'",
            send_to_discord=not silent,
            level=bot.logger.WARNING
        )
...
file_lines: List of strings. The content of the .py file, line by line
    (needed to find User and Message command descriptions).
    ...
    for member in _class.__dict__.values():
        ...
        if callable(member):
            ...
            if 'docstring' in d:
                ...
                if (not docstring.startswith("Command Group")
                        and not docstring.endswith("(Owner Only)")):
                    ...
                    if 'option' in d:
                        ...
                        for option in d['option'].options:
                            command_def["parameters"].append(
                                {
                                    "name":option.name,
                                    "description":option.description,
                                    "type":"(" + str(option.type).replace('OptionType.', '').capitalize() + ")",
                ...
                if (isinstance(d['body'], disnake.app_commands.UserCommand)
                        or isinstance(d['body'], disnake.app_commands.MessageCommand)):
            ...
            cogs.append(
                {
                    ...
                    "commands":get_commands(_class, file_content.split('\n'))
                }
            )
    return cogs
...
def writeHTMLHelp(version : str, path : str, cogs : list[dict[str, Any]]) -> None:
    ...
    filters : str = (
        ...
        ' style="background: #050505;">All</button>\n'
    )
    ...
    for cog in cogs:
        ...
        if len(cog["commands"]) > 0:
            ...
            filters += (
                '\t\t\t<button class="btn" onclick="filterSelection(\'{}\')" style="background: #{};">{}</button>\n'
            ).format(
                cog["name"].lower(),
                cog["color"],
                cog["name"]
            ...
            for command in cog["commands"]:
                ...
                containers += '\t\t\t<li class="command {}"'.format(
                    cog["name"].lower()
                ) # class
                ...
                containers += (
                    ...
                    '<span style="display: inline-block;background: #{};padding:'
                    ' 5px;text-shadow: 2px 2px 2px rgba(0,0,0,0.75);">{}</span>'
                    '&nbsp;<span style="display: inline-block;background: #{};'
                    'padding: 3px;text-shadow: 2px 2px 2px rgba(0,0,0,0.5);'
                    'font-size: 14px;">{}</span>&nbsp;&nbsp;{}'
                ).format(
                    cog["color"],
                    cog["name"],
                    cmd_color_type[command['type']],
                ...
                if command["description"] != "":
                    containers += (
                        '</div>\n'
                        '\t\t\t\t<div class="command-description"><b>Description :</b>&nbsp;{}'
                    ).format(
                        command["description"].replace(
                            '(Mod Only)', '<b>(Mod Only)</b>'
                        ).replace(
                            '((You) Mod Only)', '<b>((You) Mod Only)</b>'
                        ).replace(
                            '((You) Server Only)', '<b>((You) Server Only)</b>'
                        ).replace(
                            '(NSFW channels Only)', '<b>(NSFW channels Only)</b>'
                        )
                    )
